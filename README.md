...
콘크리트 배합 데이터(시멘트, 물, 재령 등)를 활용하여 압축강도(MPa)를 예측하는 선형 회귀(Linear Regression) 모델 구현
...
3. **특성 공학 (Feature Engineering):** 콘크리트 공학 지식을 반영한 $\ln(\text{age})$ 및 물-결합재 비($w/b$) 변수 추가를 통한 예측 오차(RMSE) 개선
